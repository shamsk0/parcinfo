...
import {
  getAllEquipements, createEquipementStandalone, updateEquipement, deleteEquipement,
  assignEquipement, unassignEquipement,
...
function EquipementListPage() {
  ...
  const handleSubmit = async (data) => {
    ...
    if (editing) {
      ...
      const currentEmployeId = editing.employe ? String(editing.employe.id) : '';
  ...
  const filtered = equipements.filter((eq) => {
    ...
    const matchesSearch =
      ...
      (eq.employe && `${eq.employe.prenom} ${eq.employe.nom}`.toLowerCase().includes(term));
          ...
          <tbody>
            {filtered.map((eq) => (
              ...
              >
                ...
                <td className="px-4 py-3"><Badge status={eq.statut} /></td>
                <td className="px-4 py-3"><Badge status={eq.etat} /></td>
                ...
                <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => handleEdit(eq)} className="text-gray-600 hover:text-steel mr-3">
                    <Pencil size={18} />
                  </button>
                  <button onClick={() => handleDelete(eq.id)} className="text-gray-600 hover:text-red-600">
                    <Trash2 size={18} />
                  </button>
                </td>
              </tr>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-6 text-center text-gray-500">Aucun équipement ne correspond</td></tr>
            )}
          </tbody>
        </table>
      </div>
